...
export default function ExperiencePanel({ experience }) {
  ...
  async function saveCurrentResonance() {
    if (!experience?.id) {
      setSaveStatus('Expérience manquante')
      return
    }
    ...
    const tagsOk = await saveEntryTags(entry.id, selectedTags)
    const linksOk = await saveEntryConnections(entry.id, connections)

    if (tagsOk && linksOk) {
      setSaveStatus('Résonance sauvegardée')
    } else {
